...
export default {
  ...
  methods: {
    ...
    sendUrl: function(gameurl) {
      ...
      location.hash = `#/new?air=1&openappstore=${gameurl}`;
    },
    //游戏跳转
    dianji: function(arr) {
      console.log('----------------------------------------------------')
      localStorage.setItem("fromPath", '/new');
      ...
      // location.hash = `#/new?air=1&direction=${direction}`;
    ...
    scrollfun: function() {
      ...
      if (scrollTop + windowHeight + 20 > scrollHeight) {
        getNew({ page: this.page++, type: Ua }).then(res => {
          var arr = res.msg;
          if (arr.length > 9) {
    ...
    var fun = () => {
      getNew({ page: this.page++, type: Ua }).then(res => {
        ...
        if (res.state == "1") {
          ...
          localForage.setItem("new_cache", this.lists);
        } else {
          this.footerTishi = true;
        }
      });
